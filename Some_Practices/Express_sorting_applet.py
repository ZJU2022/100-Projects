'''
将7.2 数据源⾥的快递信息进⾏分拣，最终⽣成的数据格式如下：
...
遍历info,取
遍历info
res[info[i][1][0:3]] = info[i]

https://blog.csdn.net/vision666/article/details/121690292
'''
...
'''
先遍历info，Province[info[i][1][0:3]]
然后set(Province),此时Province里面就是所有地方
result  = {}
for i in province:
for j in data:
双重循环，相同省份的数据都存入comm_pro。append[j]
向字典写入同一个省份的数据 result[i]=comm_pro
'''
...
print(Province)
comm_pro = []
#把相同省份的信息存入一个列表，这个list的所有value作为reslut的value
for i in Province:
    for j in info:
        if j[1][0:3] == i:
            comm_pro.append(j)
